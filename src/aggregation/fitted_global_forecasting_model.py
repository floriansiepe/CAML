...
class FittedGlobalForecastingModel(GlobalForecastingModel):
    ...
    @staticmethod
    def load_model(path: Union[str, BinaryIO], model_type) -> "GlobalForecastingModel":
        cm = load_pickle(path)
        internal_model = None
    ...
    def clever_predict(
        ...
        return self.predict(n=n, series=series, future_covariates=covariates)
        ...
        **kwargs,
        ...
        if future_covariates_copy is not None:
            ...
            future_covariates_transformed = (
                self.pipeline_future_covariates.fit_transform(future_covariates_copy)
            )
    ...
    def predict(
        ...
        if future_covariates is not None:
            if self.pipeline_future_covariates is None:
                raise RuntimeError("The model was not fitted with future covariates")
            future_covariates_scaled = self.pipeline_future_covariates.transform(
                future_covariates
            )
        if past_covariates is not None:
            raise RuntimeError("The model can not be fitted with past covariates")
